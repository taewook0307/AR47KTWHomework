...
int Plus(int _Left, int _Right)     //필요한 메모리 비용 : 8byte + a
//Plus라는 이름의 함수
//지역변수로 _Left, _right를 가짐
//해당 함수를 실행한 외부 메모리 영역에 4바이트 값을 전달
{
    return _Left + _Right;
}

void Test(int _Value) {     //필요한 메모리 비용 : 4byte + a
    _Value = 10;
    return;
}
